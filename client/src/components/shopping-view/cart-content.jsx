...
import { deleteCartItem, fetchCartItems, updateCartItemQuantity } from "../../store/shop/cart-slice";
...
const UserCartContent = ({ item }) => {
  ...
  function handleUpdateQuantity( currentItem, type){
    ...
    let newQuantity = type === "plus" ? currentItem.quantity + 1 : currentItem.quantity - 1;
    if (newQuantity < 1) {
      toast.error("Quantity cannot be less than 1");
      return;
    }
    dispatch(updateCartItemQuantity({ userId, productId, quantity: newQuantity })).then((data) => {
      if (data?.payload?.success) {
        dispatch(fetchCartItems({ userId }));
      ...
      <div className="flex-1 space-y-3">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">${Number(unitPrice).toFixed(2)} each</p>
          </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-2 py-1">
            ...
            <span className="min-w-[32px] text-center text-sm font-semibold text-slate-800">{quantity}</span>
            <Button
          ...
          <Button onClick={()=>handleDeleteItem(item)} variant="ghost" className="text-red-500 hover:bg-red-50">
